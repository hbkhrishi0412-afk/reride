...
const SuggestionIcon: React.FC<{ type: Suggestion['type'] }> = memo(({ type }) => {
    switch (type) {
        case 'pricing':
            return <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-spinny-orange" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v.01" /></svg>;
        case 'listing_quality':
            return <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-spinny-text-dark" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" /></svg>;
        case 'urgent_inquiry':
    ...
    <div className={`p-4 bg-white rounded-lg border-l-4 ${priorityStyles[suggestion.priority]}`}>
        <div className="flex items-start gap-4">
            ...
            <div className="flex-grow">
                <h4 className="font-bold text-spinny-text-dark">{suggestion.title}</h4>
                <p className="text-sm text-spinny-text-dark mt-1">{suggestion.description}</p>
            </div>
            <button
                onClick={onClick}
...
const AiAssistant: React.FC<AiAssistantProps> = ({ vehicles, conversations, onNavigateToVehicle, onNavigateToInquiry }) => {
  ...
  const renderContent = () => {
    ...
    if (error) {
      return <p className="text-center text-spinny-orange">{error}</p>;
    }

    if (suggestions.length === 0) {
      return (
        <div className="text-center py-6">
            <svg xmlns="http://www.w3.org/2000/svg" className="mx-auto h-10 w-10 text-spinny-text-dark" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z" /></svg>
            <h4 className="mt-2 font-semibold text-spinny-text-dark">All Clear!</h4>
            <p className="text-sm text-spinny-text-dark">Your AI assistant has no new suggestions right now. Great job!</p>
        </div>
      );
    }
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md">
      <h3 className="text-lg font-bold text-spinny-text-dark dark:text-spinny-text-dark mb-4 flex items-center gap-2">
        <span className="text-transparent bg-clip-text bg-gradient-to-r from-spinny-orange to-spinny-blue">✨ AI Sales Assistant</span>
      </h3>
      {renderContent()}
    </div>
  );
